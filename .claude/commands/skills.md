...
- **Tests**: If a test is added or modified, name it in full — include the `test.describe` block name and the `test()` name. For example: *"The test 'Login › should display error for locked out user' has been updated to use the new `lockedOutUser` credential constant."*
- **Methods / functions**: If a method is added or modified on a Page Object or elsewhere, name it in full. For example: *"The method `InventoryPage.addItemToCartByName()` has been added to allow tests to add a specific product to the cart by its display name."*

...
- `await expect(locator).toX()` for Playwright locator assertions (they return `Promise<void>` and auto-retry). Plain value matchers like `expect(number).toBeCloseTo()` or `expect(array).toEqual()` are synchronous — no `await`
